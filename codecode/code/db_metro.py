...
from ilu.database.db_manager import DataManager

path = '../../data/'
tablename = 'metro'
stationIDs = '1_rede/METRO/stops.txt'
dataFiles = ['1_metro/saidas_outubro_2018.xlsx','1_metro/entradas_outubro_2018.xlsx'] 
head = {
...
class MetroDataManager(DataManager):
    
    def __init__(self):
        DataManager.__init__(self,[tablename],[head])
    ...
    def transform_data(self):
        ...
        temporary = path+'1_metro/samplemetro.txt'
        ...
        with open(temporary, 'w') as f:
            ...
            for dataFile in dataFiles:
                ...
                wb = openpyxl.load_workbook(path+dataFile, read_only=True)
                
                #C: for each day (excel sheet) write records in temporary file
                for sheetname in wb.sheetnames:
                    print('Sheet:'+sheetname)
                    self.write_sheet_to_file(f, wb[sheetname], int(sheetname), isentry, stations)
                    #break
    ...
    def get_station_ids(self,):
        stationdic = {}
        with open(path+stationIDs, 'r', encoding="utf8") as lines:
            next(lines)
            for line in lines:
    ...
    def write_sheet_to_file(self, writer, wsheet, day, isentry, stations):
        ...
        for i in range(3+k,wsheet.max_row-14):
            ...
            if sheet[i][2-k].value!='': entry_station = self.get_station_id(stations,sheet[i][2-k].value)
            if entry_station is None: continue
            time = self.get_time(sheet[i][3-k].value.replace(' ',''), day)
            if time is None: continue
            for j in range(4-k,65-k):
                if sheet[1+k][j].value!='': exit_line = sheet[1+k][j].value #[6:]
                exit_station = self.get_station_id(stations,sheet[2+k][j].value)
...
if __name__ == '__main__':
    meteo_data_manager = MetroDataManager()
    meteo_data_manager.repopulate_database()
